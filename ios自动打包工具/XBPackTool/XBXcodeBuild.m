...
+ (void)xcodeBuildProjectSetBuildType:(NSString*)buildType{
    ...
    // 4.上传到蒲公英
        NSString *upload = [NSString stringWithFormat:@"curl -F file=@%@ -F uKey=%@ -F _api_key=%@ https://upload.pgyer.com/apiv1/app/upload",[NSString stringWithFormat:@"%@/%@.ipa",XB_IPAPath,XB_ProjectName],XB_PgyerUserKey,XB_PgyerAPIKey];
